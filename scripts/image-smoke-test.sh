...
RUN_ID="smoke-$$"
WEB_IMG="doelenboom-web-$RUN_ID"; EXCEL_IMG="doelenboom-excel-$RUN_ID"
WEB_C="$WEB_IMG"; EXCEL_C="$EXCEL_IMG"
WEB_PORT="${SMOKE_WEB_PORT:-18080}"; EXCEL_PORT="${SMOKE_EXCEL_PORT:-18000}"
TMP="$(mktemp -d)"

cleanup() {
  docker rm -f "$WEB_C" "$EXCEL_C" >/dev/null 2>&1 || true
  docker rmi -f "$WEB_IMG" "$EXCEL_IMG" >/dev/null 2>&1 || true
  rm -rf "$TMP"
}
trap cleanup EXIT
...
tar --exclude=node_modules --exclude=dist --exclude=.git -cf - web excel-service scripts | tar -xf - -C "$TMP"
chmod -R go-rwx "$TMP/web" "$TMP/excel-service" "$TMP/scripts"
