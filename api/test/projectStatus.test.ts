...
describe('project-status (PUT upsert + DELETE)', () => {
  ...
  it('GET .../history: before/after per veld, nieuwste eerst, touch apart gelabeld', async () => {
    // P2 is nog nooit aangeraakt -> eerste PUT heeft from=null (nog geen rij).
    ...
    const history = await req('GET', `/api/doelenbomen/${doelenboomId}/elements/P2/history`, {
      token: adminToken,
    });
    ...
    assert.equal(delRow.kind, 'status');
    assert.equal(delRow.action, 'delete');
    assert.equal(delRow.changes.projectstatus.from, 'On-hold', 'DELETE: from = wat er stond vóór het wissen');
    assert.equal(delRow.changes.projectstatus.to, null, 'DELETE: to = null (gewist)');
    ...
    assert.equal(secondRow.changes.projectstatus.from, 'Actief');
    assert.equal(secondRow.changes.projectstatus.to, 'On-hold');
    assert.equal(secondRow.changes.rag.from, 'Groen');
    assert.equal(secondRow.changes.rag.to, 'Oranje');
    assert.ok(!('toelichting' in secondRow.changes), 'ongewijzigd veld blijft onvermeld');

    assert.equal(firstRow.kind, 'status');
    assert.equal(firstRow.action, 'create');
    assert.equal(firstRow.changes.projectstatus.from, null, 'eerste keer: nog geen rij, dus from = null (niet leeg)');
    assert.equal(firstRow.changes.projectstatus.to, 'Actief');
  });

  it('een bezoeker ziet de historie wel, maar changedByEmail niet (zelfde privacy als GET .../tree)', async () => {
    const asAdmin = await req('GET', `/api/doelenbomen/${doelenboomId}/elements/P2/history`, {
      token: adminToken,
    });
    ...
    const asBezoeker = await req('GET', `/api/doelenbomen/${doelenboomId}/elements/P2/history`, {
      token: bezoekerToken,
    });
    ...
    assert.ok('changes' in asBezoeker.body[0], 'bezoeker ziet wat er gewijzigd is wel');
  });

  it('GET .../history: onbekend element geeft 404', async () => {
    const res = await req('GET', `/api/doelenbomen/${doelenboomId}/elements/GEENBESTAAND/history`, {
      token: adminToken,
    });
