...
const USER_SELECT_FIELDS = 'id, email, is_sysadmin, created_at';
...
// account aanmaken als het e-mailadres nog niet bestaat).
...
// meegegeven velden worden aangepast.
  ...
  try {
    const result = await pool.query(
      `update users set
       ...
       where id = $4
       returning ${USER_SELECT_FIELDS}`,
      [email ?? null, password ?? null, isSysadmin ?? null, userId]
    );
